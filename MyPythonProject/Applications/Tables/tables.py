...
def create_tables(db: str) -> str:
    ...
    with ExitStack() as stack:
        conn = stack.enter_context(DatabaseConnection(db))
        stack.enter_context(conn)
        conn.execute("CREATE VIEW IF NOT EXISTS tracks_vw AS "
                     "SELECT "
                     ...
                     "tra.title AS title, "
                     "COALESCE(rip.utc_ripped, NULL) AS utc_ripped, "
                     "COALESCE(pla.utc_played, NULL) AS utc_played, "
                     "COALESCE(pla.played, 0) AS played, "
                     ...
                     "dis.is_bonus AS is_disc_bonus, "
                     "tra.is_live AS is_track_live, "
                     "tra.is_bonus AS is_track_bonus, "
                     "alb.is_deluxe AS is_deluxe, "
                     "lan.language AS language, "
                     "liv.date AS live_date, "
                     "liv.city AS live_city, "
                     "liv.country AS live_country, "
                     "liv.tour AS live_tour, "
                     ...
                     "LEFT OUTER JOIN playeddiscs pla ON pla.albumid = dft.albumid AND pla.discid = dis.discid "
                     "LEFT OUTER JOIN rippeddiscs rip ON rip.albumid = dft.albumid AND rip.discid = dis.discid "
                     ...
                     "art.artist AS artist, "
                     "CASE liv.countryid "
                     "WHEN 6 THEN liv.tour || ' - ' || strftime('%d/%m/%Y', liv.date) || ' - [' || liv.city || ']' "
                     ...
                     "LEFT OUTER JOIN playeddiscs pla ON pla.albumid = boot.albumid AND pla.discid = dis.discid "
                     "LEFT OUTER JOIN rippeddiscs rip ON rip.albumid = boot.albumid AND rip.discid = dis.discid "
                     "LEFT OUTER JOIN (SELECT a.albumid, a.date, a.city, a.countryid, cou.country, a.tour FROM livealbums a INNER JOIN countries cou ON a.countryid = cou.countryid) liv "
                     "ON liv.albumid = boot.albumid "
                     "ORDER BY albumid, "
                     "discid, "
                     "trackid")
        conn.execute("CREATE VIEW IF NOT EXISTS defaultalbums_vw AS "
                     "SELECT "
                     "alb.rowid AS album_rowid, "
                     "dft.rowid AS rowid, "
                     "dft.albumid AS albumid, "
                     ...
                     "LEFT OUTER JOIN rippeddiscs rip ON rip.albumid = alb.albumid AND rip.discid = dis.discid "
                     "LEFT OUTER JOIN playeddiscs pla ON pla.albumid = alb.albumid AND pla.discid = dis.discid "
                     "ORDER BY dft.albumid, "
                     "dis.discid, "
                     "tra.trackid")
