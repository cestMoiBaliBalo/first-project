...
from Applications.shared import DATABASE, TitleCaseConverter, UTF8, eq_string, get_readabledate, get_rippingapplication, itemgetter_
...
class TestDecorator04(unittest.TestCase):
    ...
    def test_t01(self):
        ...
        _, genreid2 = next(filter(itemgetter_()(partial(eq_string, "hard rock")), get_genres(DATABASE)))
        self.assertEqual(genreid1, genreid2)

    def test_t02(self):
        _, genreid1 = list(filter(lambda i: i[0].lower() == "hard rock", get_genres(DATABASE)))[0]
        _, genreid2 = next(filter(itemgetter_()(partial(eq_string, "rock")), get_genres(DATABASE)))
