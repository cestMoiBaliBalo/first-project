...
from ..callables import filterfalse, filter_audiofiles, filter_extension, filter_extensions, filter_losslessaudiofiles, filter_portablesdocuments
...
class Test05(unittest.TestCase):
    ...
    def test04(self, mock_os_walk):
        mock_os_walk.side_effect = self.side_effect
        out_files = sorted(find_files(Path("F:/D"), excluded=filterfalse(filter_portablesdocuments)))
        self.assertListEqual(out_files, [])
        mock_os_walk.assert_called_once()

    def test05(self, mock_os_walk):
        mock_os_walk.side_effect = self.side_effect
        self.assertFalse(set(find_files(Path("F:/E"), excluded=filterfalse(filter_extensions("doc", "pdf", "txt")(filter_extension)))))
        mock_os_walk.assert_called_once()

    def test06(self, mock_os_walk):
        mock_os_walk.side_effect = self.side_effect
        out_files = sorted(find_files(Path("F:/F"), excluded=filterfalse(filter_extensions("mp3", "m4a")(filter_extension))))
    ...
    def test07(self, mock_os_walk):
        mock_os_walk.side_effect = self.side_effect
        out_files = sorted(find_files(Path("F:/G"), excluded=filterfalse(filter_losslessaudiofiles)))
        self.assertListEqual(out_files, [str(Path("G:/") / "file1.flac"), str(Path("G:/") / "file2.flac"), str(Path("G:/") / "file3.flac")])
        mock_os_walk.assert_called_once()

    def test07(self, mock_os_walk):
        mock_os_walk.side_effect = self.side_effect
        out_files = sorted(find_files(Path("F:/H"), excluded=filterfalse(filter_audiofiles)))
