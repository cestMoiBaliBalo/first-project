...
def format_collection(*iterables, start: int = 1) -> Iterator[Tuple[str, int, str]]:
    ...
    :return:
    """
    _collection: Iterator[Any] = zip(iterables, count(start))  # ((target1, 1234), 1), ((target2, 5678), 2), ...
    _collection = iter(tuple(chain(*[target], (number,))) for target, number in _collection)  # (target1, 1234, 1), (target2, 5678, 2), ...
...
def format_menu(*iterables, group=3) -> Iterator[str]:
    ...
    :return:
    """
    collection: Iterator[Any] = iter(tuple(compress(item, [1, 1, 0])) for item in iterables)  # ("menu1", 1), ("menu2", 2), (None, None), ...
    collection = starmap(_format1, collection)  # " 1. menu1", " 2. menu2", "", ...
    collection = zip_longest(*[collection] * group)  # (" 1. menu1", " 2. menu2", ""), ...
    collection = iter(tuple(map(_format2, item)) for item in collection)  # (" 1. menu1     ", " 2. menu2     ", "          "), ...
    collection = iter("".join(item) for item in collection)  # " 1. menu1      2. menu2               ", ...
