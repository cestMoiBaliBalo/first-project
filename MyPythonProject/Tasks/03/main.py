...
from Applications.shared import ChangeLocalCurrentDirectory, MUSIC, TemplatingEnvironment, cjustify, get_input, rjustify
...
def pprint_files(*iterables, length=229):
    iterables = iter(iterables)
    it1, it2 = tee(iterables)
    keys = islice(zip(*it1), 1)
    pairs = islice(zip(*it2), 1, 2)
...
while not q_pressed:
    ...
    while not q_pressed:
        ...
        answer = get_input(prompt, template=T1, cwd=folder, menu=menu, collection1=[], collection2=[], collection3=[])
        if answer.lower() == "q":
            q_pressed = True
    ...
    if d_pressed:
        ...
        collection1 = pprint_files(*list((file, os.path.getsize(file)) for file in collection))
        collection3 = pprint_extensions(*list((key.upper(), len(list(group))) for key, group in groupby(sorted(os.path.splitext(file)[1][1:] for file in collection))))
        while True:
            get_input(" Press any key to continue...", template=T1, cwd=folder, menu=[], collection1=collection1, collection2=collection2, collection3=collection3)
            break
        folder, level, d_pressed, q_pressed, collection, collection2 = init()
